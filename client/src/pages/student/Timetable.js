...
export default class Timetable extends React.PureComponent {
    ...
    componentDidMount() {
        ...
        fetch(`http://localhost:3001/student/timetable/${email}`, {
            cache: 'no-cache',
        })
            .then((res) => res.json())
            .then((data) =>
                this.setState(() => ({
                    data: data.timetable.map( (raw_data, i)=> ({title: raw_data.name, startDate: new Date(raw_data.year,raw_data.month-1,raw_data.day,raw_data.StartTime,0),endDate: new Date(raw_data.year,raw_data.month-1,raw_data.day,raw_data.EndTime,0) ,location: raw_data.Room, id: i})),
                }))
            );
    }
    render() {
